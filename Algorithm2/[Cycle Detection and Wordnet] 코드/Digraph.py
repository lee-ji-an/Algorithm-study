...
def sap(g, aList, bList):
    ...
    for item in aList:
        visitA.append((item, 0))
        q.put((item, 'a', d))
    for item in bList:
        visitB.append((item, 0))
        q.put((item, 'b', d))

    sapLength = [-1, math.inf]
    while not q.empty():
        num, lname, dist = q.get()
        if dist > sapLength[1]:
            break
        d = dist + 1
        for w in g.adj[num]:
            if lname == 'a':
                ...
                for item in visitA:
                    if item[0] == w:
                        flag = False
                        tempA = item[1]
                if flag:  # a가 방문한 적이 없었다면
                    visitA.append((w, d))
                    q.put((w, 'a', d))
                    tempA = d
                for item in visitB:
                    if item[0] == w:
                        if sapLength[1] > tempA + item[1]:
                            sapLength = [item[0], tempA + item[1]]
            elif lname == 'b':
                flag = True
                for item in visitB:
                    if item[0] == w:
                        flag = False
                        tempB = item[1]
                if flag:  # b가 방문한 적이 없었다면
                    visitB.append((w, d))
                    q.put((w, 'b', d))
                    tempB = d
                for item in visitA:
                    if item[0] == w:
                        if sapLength[1] > tempB + item[1]:
                            sapLength = [item[0], tempB + item[1]]
    return sapLength[0], sapLength[1]
