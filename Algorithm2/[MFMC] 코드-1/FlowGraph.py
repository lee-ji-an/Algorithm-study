...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        for item in self.teams:
            if item != teamName:
                id = self.team2id[item]
                if self.wins[teamId] + self.remaining[teamId] < self.wins[id]:
                    resultTeam.append(item)
        if len(resultTeam) > 0: return True, resultTeam
        length = len(self.teams)
        graph = FlowNetwork(length*(length+1)//2+2)
        idx = 0
        midIdx = length*(length-1)//2
        for i in range(length):
            for j in range(i+1, length):
                idx += 1
                if (j != teamId) and (i != teamId):
                    graph.addEdge(FlowEdge(0, idx, self.against[i][j]))
                    graph.addEdge(FlowEdge(idx, midIdx+i+1, float('inf')))
                    graph.addEdge(FlowEdge(idx, midIdx+j+1, float('inf')))
        endIdx = midIdx + length + 1
        for i in range(0, length):
            if (i != teamId):
                graph.addEdge(FlowEdge(midIdx+i+1, endIdx, self.wins[teamId] + self.remaining[teamId] - self.wins[i]))
        ff = FordFulkerson(graph, 0, graph.V-1)

        for i in range(length):
            if ff.inCut(midIdx+i+1) and i != teamId:
                resultTeam.append(self.teams[i])
        if len(resultTeam) > 0:
            return True, resultTeam
        else:
            return False, []
