...
def mstPrimEager(g):
    def include(v):
        ...
        for e in g.adj[v]:
            if not included[e.other(v)]:
                ...
                included[e.other(v)] = True
            else:
                if not edgeList[e.other(v)]:
                    w = pq.keyOf(e.other(v)).weight
                    if e.weight < w:
    ...
    weightSum = 0
    include(0)
    while not pq.size() == 0 and len(edgesInMST) < g.V - 1:
        e, w = pq.delMin()
        edgesInMST.append(e)
        weightSum += e.weight
        if not edgeList[e.v]: edgeList[e.v] = True  # Add to the MST the vertex not yet included
        if not edgeList[e.w]: edgeList[e.w] = True
        include(w)
