            ...
            for x in range(self.width()):
                if x == 0:
                    if distTo[y - 1][x] > distTo[y - 1][x + 1]:
                        minValue = distTo[y - 1][x + 1]
                        idx = x + 1
                    else:
                        minValue = distTo[y - 1][x]
                        idx = x
                elif x == self.width() - 1:
                    if distTo[y - 1][x] > distTo[y - 1][x - 1]:
                        minValue = distTo[y - 1][x - 1]
                ...
                distTo[y][x] = minValue + self.energy(x, y)
                edgeTo[y][x] = idx

        result = []
        minResult = distTo[self.height()-1][0]
        minCol = 0
        for i in range(1, self.width()):
            if minResult > distTo[self.height()-1][i]:
                minResult = distTo[self.height()-1][i]
                minCol = i
        for y in range(self.height()-1, -1, -1):
            result.append(minCol)
            minCol = edgeTo[y][minCol]
        return result
