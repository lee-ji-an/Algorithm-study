...
def solveManhattan(initialBoard):
    ...
    que = PriorityQueue()
    que.put((initialBoard.manhattan(), initialBoard, 0, None))
    que.put((initialBoard.hamming(), initialBoard, 0, None))
    res_list = []
    while True:
        minPQ = que.get()
        if minPQ[1].isGoal():
            while True:
                res_list.insert(0, minPQ[1])
                if minPQ[3] is None:
                    break
                minPQ = minPQ[3]
            return res_list
        else:
            n_list = minPQ[1].neighbors()
            for i in range(len(n_list)):
                if minPQ[3] is None or n_list[i] != minPQ[3][1]:
                    que.put((minPQ[2]+1+n_list[i].manhattan(), n_list[i], minPQ[2]+1, minPQ))
                    # que.put((minPQ[2] + 1 + n_list[i].hamming(), n_list[i], minPQ[2] + 1, minPQ))
