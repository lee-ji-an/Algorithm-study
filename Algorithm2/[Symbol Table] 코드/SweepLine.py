...
def sweepLine(segments):
    ...
    for item in segments:
        que.put((item.x1, item.y1, item))
        if item.x1 != item.x2:
            que.put((item.x2, item.y1, item))

    while not que.empty():
        point = que.get()
        if point[2].isHorizontal():
            if llrb.contains(point[2].y1):
                llrb.delete(point[2].y1)
            else:
                llrb.put(point[2].y1, point[2])
        elif point[2].isVertical():
            inter = llrb.rangeSearch(point[2].y1, point[2].y2)
            for i in range(len(inter)):
                res.append((llrb.get(inter[i]), point[2]))
                # print(res)
    return res
