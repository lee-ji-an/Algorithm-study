...
class SCC:
    def __init__(self, g): # Do strongly-connected-components pre-processing, based on Kosaraju-Sharir algorithm
        self.g = g
        self.count = 0  #component 개수 저장
        self.id = [-1] * g.V    #component id 저장 -> 같은 id = 같은 component
        g2 = Digraph.reverse(g)
        g2 = topologicalSort(g2)
        visited = [False for _ in range(g.V)] # 노드 방문 여부 저장하는 리스트
        for i in g2:
            if not visited[i]:
                comp = BFS(g, i)
                for j in range(len(comp.visited)):          #comp.visited -> connected 노드 : 1, not connected : 0
                    if not visited[j] and comp.visited[j]:  # 아직 방문 x and i에서 도달 가능한 노드일 때
                        self.id[j] = self.count             # 같은 id 부여
                        visited[j] = True
                self.count += 1
        count_list = []
        for i in range(g.V):
            if self.id[i] not in count_list:
                count_list.append(self.id[i])
        self.count = len(count_list)
